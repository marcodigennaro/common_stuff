...
def now_running():
    return int( sp.check_output( 'squeue | grep mdi0316 | wc -l', shell=True ).decode("utf-8"))

def bash_command_to_list(cmd):
    process = sp.Popen(['/bin/bash', '-c', cmd], stdout=sp.PIPE)
    out_list = [ l.decode("utf-8").strip() for l in process.stdout.readlines() ]
    return out_list 
